  ...
  useEffect(() => {
    ...
    async function fetchSettings() {
      try {
        ...
        if (isMounted && showError) showError("Failed to load settings");
      } finally {
    ...
    <div className="max-w-3xl mx-auto animate-in fade-in zoom-in-95 duration-500">
      ...
      <form onSubmit={initiateSave} className="space-y-6">
        ...
        <Button
          type="submit"
          disabled={saving}
          className="w-full h-14 bg-(--color-gold) text-black font-bold rounded-xl hover:bg-[#b89445] transition-all shadow-[0_0_20px_rgba(197,160,89,0.2)] text-base"
        >
          {saving ? (
