...
const ProductModal = ({ product, isOpen, setIsOpen, trigger }) => {
    ...
    const generateShareLink = () => {
        if (typeof window !== "undefined") {
            // URL banega: domain.com/?product=1 (id ke base par)
                ...
                <div className="flex flex-col md:flex-row flex-1 min-h-0">
                    ...
                    <div className="relative w-full md:w-[45%] h-40 md:h-auto shrink-0 bg-black/50">
                        ...
                        <div className="absolute top-3 left-3 md:hidden z-20">
                            <button onClick={() => setShowShareMenu(!showShareMenu)} className="bg-black/40 backdrop-blur-md p-2 rounded-full text-white border border-white/10">
                                <Share2 className="w-4 h-4" />
                            </button>
                            {showShareMenu && <ShareMenu onShare={handleShare} onCopy={handleCopyLink} copied={copied} />}
                        </div>
                        <button onClick={() => setIsOpen(false)} className="absolute top-3 right-3 md:hidden z-20 bg-black/40 backdrop-blur-md p-2 rounded-full text-white border border-white/10">
                            <X className="w-4 h-4" />
                        </button>
                    </div>
